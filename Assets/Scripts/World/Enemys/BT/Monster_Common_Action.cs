...
namespace World.Enemys.BT
{
    public interface IEnemy_Can_Shoot
    {
        ...
        void Monster_Shoot(Enemy actor, string muzzle_bone_name, Vector2 expected_pos, float target_v)
        {
            ...
            Enemy_Shoot_Helper.do_shoot(actor, muzzle_pos, shoot_dir, Projectile_Speed);
            Shoot_Finished = true;
    ...
    {
        private const float CHASING_DISTANCE_MIN = -6f;
        private const float CHASING_DISTANCE_MAX = 8f;
        ...
        protected void Set_Chasing_Speed(Enemy cell, float v_base, float v_max)   // 2025.03.24 关卡追逐优化
        {
            ...
            var pos_delta = Forced_Get_Target_Pos().x - cell.pos.x;
            if (pos_delta <= CHASING_DISTANCE_MIN)
            {
                set_v(v_base);
                return;
            }

            float chasing_distance = CHASING_DISTANCE_MIN + (CHASING_DISTANCE_MAX - CHASING_DISTANCE_MIN) / Mathf.Min(v_delta, v_max - v_base) * target_velocity_x;
            float v = v_base + v_delta / (chasing_distance - CHASING_DISTANCE_MIN) * (pos_delta - CHASING_DISTANCE_MIN);  // y = y1 + (y2-y1)/(x2-x1) * (x-x1)
            set_v(v);
